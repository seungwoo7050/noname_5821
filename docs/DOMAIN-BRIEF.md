...
- Open questions that do not block implementation: Final product name; the first real games and platforms; the permitted real-observation sources; whether editions are separate games or releases; future outlier and minimum-sample rules; public contribution and account policy; backlog scope; monetization; production hosting; and trademark or domain availability. The repository is the approved public `seungwoo7050/audience-foundry-clear-time` repository. No missing or contradictory decision blocks the local MVP loop because third-party imports, public mutation, payment, and production deployment are explicitly excluded.
